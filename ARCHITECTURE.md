...
**Culling matters.** The world is nine viewport-widths long, so at the start and on a runaway much of the field is off camera. `visibleWorldRange()` gates the horses and the track furniture. On a phone, though, the field is pulled in (`WORLD.spreadScale`) and mid-race all 24 are usually in shot, so what a phone pays is the cost of a horse — see "What a frame costs" in § 6.6.
...
- **Cheap enough**: canvas and simple geometry, drawn to as few draw calls as give the same picture (see "What a frame costs" below).
...
- **Volume** — the coat is lit: warm along the topline with a rim of sunlight, sheen over the quarters and shoulder, dark under the barrel, creases at the stifle and behind the elbow. Forearm and gaskin are tapered muscle shapes (`drawLegPair()` / `taperPath()`), not strokes, with joint bulges at knee, hock and fetlock, a pastern sloping into the hoof, and a tendon line down the back of the cannon. The far-side legs are drawn first in the shade colour so the near pair reads in front of them.
...
**Shading is cheap because the gradients are cached** (`horseGrad()`). A canvas gradient is defined in user space and read through whatever transform is current when it is used, so one gradient in local horse coordinates serves every horse in every frame. Building them per horse per frame would be over a thousand allocations a second for nothing.

**What a frame costs.** Measured in Chrome with DevTools' phone emulation (393×852 at 3×), both the GPU and the main thread pay per *draw call*, not per pixel: rendering the canvases with a quarter of the pixels changed nothing, while dropping half the fills took the race from 32 to 52fps. A full field was about 1,340 fills, 216 strokes and 96 clips a frame, nearly all of it horses. So a horse is drawn with the fewest calls that give the same picture:

- **One fill per colour, where the shapes of a colour are drawn together.** A side's two legs are one fill per colour (`drawLegPair()`: the hind and the fore never touch through the stride, and a unit test checks it); so are the mane's strands, the jockey's thigh and boots and his arm, a silk's hoops or stripes, the foreground grass and the rail posts. Merging shapes has one trap: under the non-zero rule, overlapping shapes traced in opposite directions cancel and leave a hole, so `taperPath()` and `dotPath()` both run anticlockwise — also tested.
- **No clips for shading.** A shading layer is the outline filled with its gradient rather than a rectangle clipped to the outline; the picture is the same because each outline sits inside its layer's rectangle and the radial sheens fade out inside it. Clips stay only where a shape really runs off the edge: a face stripe, a grey's dapples, a silk pattern.
- **A horse's text is an image.** Every `fillText` is laid out and turned into glyphs afresh, and under a horse's moving transform nothing can be reused: 24 saddle-cloth numbers were a third of a throttled phone's frame. Each horse's number, and a starred silk, is set once per size — 1, 2, 4 and 8 pixels to the unit — and the size just finer than the screen is drawn with plain bilinear filtering (`horseTextImage()`). Not `imageSmoothingQuality = 'high'`: mipmapped sampling cost the GPU more than the text had cost the CPU. The Winning Moment's close-up and the track lettering stay live text; they are big enough that a bitmap would read a shade softer.
- **A save only to undo a transform.** Opacity is put back by hand, and the jockey's upper body and head save the canvas only for the salute.

Back to back on the same machine, with the committed engine against these changes: mid-race went from 42 to 58fps in plain phone emulation and from 10 to 17fps with 4× CPU throttling, and Skip to Finish answers in about 110ms rather than 180ms. The pictures differ only in anti-aliasing where merged shapes meet (0.2–0.3% of pixels by more than 8 levels of 255). A drawing change that adds draw calls, clips or text to the horse is a performance change and wants measuring the same way.
...
- **`npm test`** — two suites on Node's own test runner:
  - `tests/engine.test.js`, the engine's pure logic: the margin parser and formatters, ordinals, the invented finishing gaps (monotonic, a close finish), the replay's finishing order (ids as numbers or strings, and the fall-back to the forecast when none match), the start ease and the run-through curves, the card's pause, the "a planted hoof cannot skate" invariant of the leg rig, the IK, the two rules that let a horse's shapes share a fill (a side's legs never touch; joints and limbs wind the same way), commentary templating, the config merge and `esc()`. `tests/load-engine.js` loads the real `flat.js` into a Node `vm` against an inert browser and calls `FlatEngine.internals`.
