...
├── js/
│   ├── flat.js             ← THE ENGINE, GENERATED by tools/build.js — do not edit; this is the file production copies
│   ├── src/                ← the engine's source, in reading order; edit HERE, then `npm run build`
│   │   ├── 00-banner.txt   ← the file header the build stamps "GENERATED" onto
│   │   ├── 10-config.js    ← seed defaults, the commentary script, prefers-reduced-motion
│   │   ├── 20-data-shapes.js   ← the payload typedefs
│   │   ├── 30-state-world.js   ← state, canvas + DPI, viewport and world layout
│   │   ├── 40-flow.js      ← the director, init, screens, narration, intro, parade
│   │   ├── 50-race.js      ← race start, the race model, the master timeline, the camera
│   │   ├── 60-scenery.js   ← painting helpers, crowd, clouds, grandstands, trees, hoardings
│   │   ├── 65-world-paint.js   ← sky, track plane, the finish, hoof dust, flashguns, the render loop
│   │   ├── 70-horse.js     ← the gait rig, the two-bone IK, and drawing a horse and jockey
│   │   ├── 80-commentary.js
│   │   ├── 85-leaderboard.js / 86-leaderboard-rows.js
│   │   ├── 90-result.js    ← crossing the line, the margin, the result card
│   │   ├── 92-reveal.js    ← the Winning Moment, roll call, reveal, replay
│   │   └── 99-api.js       ← the public API and first layout
│   ├── shared/             ← used by BOTH engines; one copy, no drift
│   │   ├── payload-text.js ← esc() — data does not get to write HTML
│   │   └── silks.js        ← the silk badge and jockey-cap renderers
...
├── tools/
│   ├── build.js             ← concatenates js/shared + js/src into js/flat.js
...
└── fonts/
    ...
    └── *.woff2             ← three VARIABLE fonts: Playfair (upright + italic) and DM Sans
```

**Why a build?** The engine ships as one classic script — production includes
it with a plain `<script>` from `<head>`, and § 9's promise is "copy the file,
done". An ES module would defer past the inline boot script that calls
`init()`. So the *source* is split and the *artefact* stays one file.
`tools/build.js` concatenates the manifest in order; the fragments share one
function scope inside the IIFE exactly as they did when they were one file, so
nothing needed import/export rewiring. Edit a fragment, run `npm run build`.
`npm test` fails if `js/flat.js` and the fragments disagree.
